...
def _patched_get(*args, **kwargs):
  kwargs['cookies'] = krx_auth._session.cookies  # 예시: krx_auth의 쿠키 주입
...
def buy_condition(df):
  ...
  return ((df['RSI'] <= DEFAULT_RSI_THRESHOLD) & (~df['Bullish']) & (df['Close'].shift(1) - df['Close'] > df['ATR'].shift(1) * 1.5)) | \
         (df['Change_Rate'] < -5)
