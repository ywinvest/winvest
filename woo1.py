...
def buy_condition(df):
  ...
  conditions &= (df['High_Change'].shift(1) >= 10)
  conditions &= (df['Bullish'].shift(1))
  conditions &= (df['Volume_Change'].shift(1) > 4) # 300% 초과
  ...
  conditions &= ((df['High_Change'].shift(1) - df['Change'].shift(1)) > 0.1 or (df['High_Change'].shift(1) - df['Change'].shift(1)) <= 0.025) # 52주 신저가 경신 제외
  return conditions
