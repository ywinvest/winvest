...
def get_index_rsi(index_code, today, period=14):
  """
  주어진 지수의 오늘 날짜 RSI를 계산하는 함수
  """
  # 오늘 날짜 가져오기
  # today = datetime.now().strftime('%Y-%m-%d')

  # 오늘 날짜 데이터만 불러오기
  df = fdr.DataReader(index_code, today)

  if df.empty:
    print(f"({index_code}) 오늘 날짜 데이터가 없습니다.")
    return None

  # ta 라이브러리를 이용해 RSI 계산
  df['RSI'] = ta.rsi(df['Close'], length=int(period))

  # 오늘 날짜의 RSI 가져오기
  today_rsi = df['RSI'].iloc[-1]
  print(f"({index_code}) 오늘 날짜의 RSI ({today} 기준): {today_rsi:.2f}")
  return today_rsi
