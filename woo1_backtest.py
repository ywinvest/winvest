  ...
  try:
    ...
    if not buys.empty:
      for buy_date in buys.index:
        ...
        data_1 = df.loc[buy_date + timedelta(days=1):]
        ...
        # 2~5일차 (T+2 ~ T+5)
        # if not partial_sell_date:
        #   data_2_5 = df.loc[buy_date + timedelta(days=1):buy_date + timedelta(days=5)]
        #
        #   if not data_2_5.empty:
        #     # 각 조건이 처음 발생하는 날짜 찾기
        #     target_open_sell = data_2_5[data_2_5['Open'] >= buy_price * PARTIAL_TARGET_RETURN]
        #     target_high_sell = data_2_5[(data_2_5['Open'] < buy_price * PARTIAL_TARGET_RETURN) & (data_2_5['High'] >= buy_price * PARTIAL_TARGET_RETURN)]
        #     stop_loss = data_2_5[(data_2_5['Close'] < df.loc[buy_date, 'Open']) & (data_2_5['Close'] < data_2_5['MA20']) & ~data_2_5['Bullish']]
        #
        ...
        #         partial_sell_price = data_2_5.loc[earliest_date, 'Open']
        #       elif condition == 'high':
        #         # 고가 5% 이상 시 5%에 부분매도
        #         partial_sell_date = earliest_date
        #         partial_sell_price = buy_price * PARTIAL_TARGET_RETURN
        ...
        # 6~22일차 (T+6 ~ T+22) - 목표 매도가를 매수가로 변경
        if not partial_sell_date:
          data_6_22 = df.loc[buy_date + timedelta(days=1):buy_date + timedelta(days=22)]

          if not data_6_22.empty:
            # # 매수일로부터 22일 후의 날짜 계산
            # max_hold_date = None
            # trading_days = 0
            # for date in data_6_22.index:
            #   trading_days += 1
            #   if trading_days >= 21:
            #     max_hold_date = date
            #     break
            ...
            # 각 조건이 처음 발생하는 날짜 찾기
            target_open_sell = data_6_22[data_6_22['Open'] >= buy_price]
            target_high_sell = data_6_22[(data_6_22['Open'] < buy_price) & (data_6_22['High'] >= buy_price)]
